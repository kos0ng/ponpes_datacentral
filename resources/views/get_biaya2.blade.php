                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body card-block">
                                        <form action="/data_santri/update_getbiaya" method="post" enctype="multipart/form-data" class="form-horizontal">
                                            ...
                                            <div class="row form-group">
                                                ...
                                                <div class="col-12 col-md-9">
                                                    ...
                                                    <option value="{{$row->id."-".$row2->id_biaya}}" {{ (  $row2->id_biaya == $row->id_biaya ) ? 'selected' : '' }}> {{ $row2->kategori." - Kelas ".$row2->sub_kategori  }}</option>
                                                    @endforeach
                                                    </select>
                                                    <select name="santri_baru[]" class="form-control" style="margin-top: 2%">
                                                    @for($i=0;$i<2;$i++)
                                                        <option value="{{$row->id."-".$i}}" {{ (  $i == $row->sb ) ? 'selected' : '' }}>{{ $arr[$i] }}</option>
                                                    @endfor
                                                    </select>
                                                </div>
                                            </div>
                                        ...
                                        $data=DB::table('biaya')->where('id_lembaga',$row->id_pondok)->orderBy('id_lembaga','DESC')->get();
                                                    ...
                                                    @foreach($data as $row2)
                                                        <option value="{{$row->id."-".$row2->id_biaya}}"  {{ (  $row2->id_biaya == $row->id_biaya ) ? 'selected' : '' }}>{{ $row2->kategori }}</option>
                                                    @endforeach
                                                    </select>
                                                    <select name="santri_baru[]" class="form-control" style="margin-top: 2%">
                                                    @for($i=0;$i<2;$i++)
